...
doc_export: ## Команда для экспорта документации
	$(xcrun --find docc) process-archive transform-for-static-hosting ./DerivedData/Build/Demo.doccarchive --output-path docs --hosting-base-path Demo
	$(xcrun --find docc) process-archive transform-for-static-hosting ./DerivedData/Build/DemoTutorials.doccarchive --output-path docs --hosting-base-path Demo
